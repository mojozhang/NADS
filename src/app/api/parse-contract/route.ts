...
6. **核心逻辑拆解**：针对提取出的交货期条款，请进一步拆解：
   - deliveryType: 如果明确提到 "工作日"，填 "workday"；如果是 "天" 或 "日" 且未提工作日，或者提到 "自然日"，填 "natural"；如果是具体日期（如2025-10-1），填 "absolute"。
    ...
    try {
        ...
        const project = await (prisma.project as any).create({
            data: {
                ...
                delivery: parsedData.deliveryType === "absolute" && parsedData.deliveryRaw?.match(/\d{4}-\d{1,2}-\d{1,2}/)
                    ? new Date(parsedData.deliveryRaw.match(/\d{4}-\d{1,2}-\d{1,2}/)[0])
                    : null,
