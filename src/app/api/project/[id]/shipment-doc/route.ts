...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        if (!project) {
            log(`DEBUG: Project not found: ${params.id}`);
            return new Response("Project not found", { status: 404 })
        }

        log(`DEBUG: Project found: ${project.name}, devices count: ${project.devices.length}`);

        // 只筛选已发货的设备
        const shippedDevices = project.devices.filter((d: any) => d.shipmentAck !== null && d.shipmentAck !== undefined)

        log(`DEBUG: Shipped devices count: ${shippedDevices.length}`);

        if (shippedDevices.length === 0) {
            log(`DEBUG: No shipped devices found, returning 400`);
            return new Response("没有已发货的设备，请先在界面上勾选发货圆点并确认。", {
                status: 400,
        ...
        const deviceRows = shippedDevices.map((device: any, index: number) =>
            new TableRow({
                children: [
        ...
        const emptyRowCount = Math.max(0, 5 - shippedDevices.length)
        const emptyRows = Array.from({ length: emptyRowCount }, (_, i) =>
            new TableRow({
                children: [
                    new TableCell({ children: [new Paragraph({ text: String(shippedDevices.length + i + 1), alignment: AlignmentType.CENTER })] }),
        ...
        log(`DEBUG: Document buffer generated, size: ${buffer.length}`);

        const rawFileName = `shipment_order_${contractNumber || params.id}.docx`
        const encodedFileName = encodeURIComponent(`发货单_${contractNumber || '未命名'}.docx`)
        ...
        log(`DEBUG: CRITICAL ERROR: ${e.message}\n${e.stack}`);
